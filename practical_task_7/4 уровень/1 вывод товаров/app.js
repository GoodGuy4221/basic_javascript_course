...
function clickHandler(event) {
    ...
    const typeCategory = event.target.dataset.type;
...
function showCategory(category) {
    ...
    let html = getProductMarkup(categoryArray);
    let divProducts = document.querySelector('.products');
    divProducts.insertAdjacentHTML('afterbegin', html);
}
...
function getProductMarkup(product) {
    ...
    <div>${obj.price}</div>
    <a href="https://example.com/producs/${obj.id}">Подробнее</a>
    </div>`
    }
